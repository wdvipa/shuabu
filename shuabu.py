...
//例如:
13*********0, 123 ,1684-11660
13*********1, 123 ,22660-31660
//如需推送将需要的推送写入变量shuabu_fs即可多个用&隔开
如:变量内输入push需再添加shuabu_push变量 内容是push的token即可
"""
...
# 检测推送
if cs == 1:
    if "cs_kdl" in os.environ:
        datas = os.environ.get("cs_kdl")
    else:
        print('您没有输入任何信息')
        exit
elif cs == 2:
    ...
    if "shuabu_fs" in os.environ:
        fs = os.environ.get('shuabu_fs')
        fss = fs.split("&")
        if "tel" in fss:
            if "shuabu_telkey" in os.environ:
                telekey = os.environ.get("shuabu_telkey")
                telekeys = telekey.split('\n')
                ttoken = telekeys[0]
                tuserid = telekeys[1]
        if "qm" in fss:
            if "shuabu_qkey" in os.environ:
                QKey = os.environ.get("shuabu_qkey")
        if "stb" in fss:
            if "shuabu_skey" in os.environ:
                SKey = os.environ.get("shuabu_skey")
        if "push" in fss:
            if "shuabu_push" in os.environ:
                push_token = os.environ.get("shuabu_push")
        if "kt" in fss:
            if "shuabu_ktkey" in os.environ:
                ktkey = os.environ.get("shuabu_ktkey")
    if "shuabu" in os.environ:
        datas = os.environ.get("shuabu")
    else:
        print('您没有输入任何信息')
        exit()
groups = datas.split('\n')
...
class shuabuanelQd(object):
    def __init__(self, user, password, step):
        # 账号
        self.user = user
        # 密码
    ...
    def login(self):
        ...
        r1 = session.post(url1, data=data1, allow_redirects=False)
        print(r1.text)
        self.msg = self.msg + "\n" + r1.text
    ...
    def main(self):
        ...
        data_json = re.sub(findstep.findall(data_json)[0], self.step, str(data_json))
        ...
        print(response)
        self.msg = self.msg + str(response)
        result = response['message'] + f"修改步数为: {self.step}  "
        print(result)
        self.msg = self.msg + "\n" + str(result)
        msgs = msgs + '\n' + self.msg
...
while i < len(groups):
    ...
    user = profile[0]
    password = profile[1]
    steps = profile[2].split('-')
    step = str(randint(int(steps[0]), int(steps[1])))
    msgs = msgs + '\n' + "用户 " + user + " 的签到结果"
    print("第" + str(n) + "个用户 " + user + " 开始签到")
    session = requests.session()
    session.headers = {
        'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 9; MI 6 MIUI/20.6.18)'
    }
    run = shuabuanelQd(user, password, step)
    run.main()
    time.sleep(5)
    i += 1
else:
    # shuabuanelQd.server_send( msgs )
    shuabuanelQd.kt_send(msgs)
    # shuabuanelQd.Qmsg_send(shuabuanelQd.name+"\n"+shuabuanelQd.email+"\n"+ msgs)
    # shuabuanelQd.tele_send(shuabuanelQd.name+"\n"+shuabuanelQd.email+"\n"+ msgs)
    shuabuanelQd.pushplus_send(msgs)
